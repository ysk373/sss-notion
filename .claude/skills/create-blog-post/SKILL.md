...
description: |
Notionで新しいブログ記事を作成するワークフロー。
ユーザーが「ブログを書いて」「記事を作成して」と言った時に使用する。
Notion MCPを使用してページ作成・プロパティ設定・本文追加を一連で実行する。
context: fork
...
- **絵文字**: 文末や強調したい箇所に積極的に使用
- **チェックリスト**: 冒頭の「この記事でわかること」や「まとめ」はチェックマークを使ったリスト形式にする。
- **Calloutブロック**: 補足やヒントはCalloutブロックを使用。
...
- `GenerateImage` が使えない環境では、`nano-banana-pro` の手順どおり `generate-image.js` を使って生成する。
- 図解は 16:9 を基本にし、記事本文の流れ（全体像→手順→比較）に対応させる。
- プロンプトには「要素数」「矢印本数」「ラベル文言」「No extra lines/boxes」を明記して、余計な図形を防ぐ。
- 生成した画像の保存先は原則 `public/notion/` 配下とし、ファイル名はスラッグに合わせる（例: `my-post-01-overview.png`）。
- 本文作成時は、画像の挿入位置（どの見出しの直後か）を明示する。
