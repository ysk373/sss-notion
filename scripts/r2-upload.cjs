...
const R2_PUBLIC_URL =
  process.env.R2_PUBLIC_URL || 'https://images.sssstudy.com';

// 設定確認
if (!R2_ACCOUNT_ID || !R2_ACCESS_KEY_ID || !R2_SECRET_ACCESS_KEY) {
  console.error('エラー: R2の環境変数が設定されていません。');
  console.error(
    '必要な環境変数: R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY'
  );
  process.exit(1);
}
...
async function uploadToR2(filePath, destinationPath = null) {
  try {
    ...
    // 保存先パスの決定
    const key =
      destinationPath || generateUniqueFileName(path.basename(filePath));
...
async function main() {
  ...
  if (args.length === 0) {
    console.log(
      '使用方法: node scripts/r2-upload.cjs <ファイルパス> [保存先パス]'
    );
    console.log('');
    console.log('例:');
